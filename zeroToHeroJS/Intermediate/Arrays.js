...
//In JS when we make copy of an array shallow copies(of an object is a copy whose properties share the same references(point to the same underlying values) as those of the source object from which the copy are made) are created.
myArr.push(6);  //Value added in array.
myArr.pop();    //Removes the last value of array
myArr.unshift(9); //Used to add value in array from beginning. ex od such thing is when you add anything in the top of your to-do list.
...
//Interview -- But the only difference between splice and slice the above instead.. slice keeps the original array intact but splice changes(removes the part of array) the original array
